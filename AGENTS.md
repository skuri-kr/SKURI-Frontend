...
- 읽기 전용 조사, 설명, 상태 보고처럼 저장소를 수정하지 않는 작업 또는 작은 변경(파일 변경 5개 이하 또는 LOC 50 Line 이하)에는 브랜치가 필요 없다.
...
아래 원칙은 [Karpathy Guidelines](https://github.com/multica-ai/andrej-karpathy-skills/blob/main/CLAUDE.md)를 이 프로젝트의 보안·검증·Git 절차에 맞게 재구성한 것이다. 이 문서에 이미 있는 명시적인 규칙과 사용자 요청이 우선한다.

### 사전 판단과 명확화

- 구현 전에 요구사항, 변경 범위, 가정을 간단히 밝힌다.
- 여러 해석이 가능하고 선택이 사용자 경험, API·DB 계약, 보안·권한, 배포에 영향을 주면 해석과 대안을 제시하고 사용자에게 확인받는다. 사소하고 되돌릴 수 있는 구현 세부는 합리적으로 가정하되, 최종 보고에 남긴다.
- 더 단순한 해결 방법이 있거나 요청이 필요 이상의 복잡성을 유발하면 그 이유와 대안을 보고한다. 요청 범위를 변경해야 하는 경우에는 승인 전에 구현하지 않는다.

### 최소·국소 변경

- 요청받지 않은 기능, 추상화, 설정 가능성, 인접 코드 정리 또는 리팩터링을 추가하지 않는다.
- 기존 코드의 스타일과 구조를 따른다. 변경으로 인해 새로 미사용이 된 import·변수·함수만 정리한다.
- 기존의 미사용 코드나 결함을 발견하면 보고하되, 별도 요청 없이 삭제하거나 수정하지 않는다.
- 모든 변경은 사용자의 요청 또는 이를 충족하기 위한 직접적인 근거를 가져야 한다.

### 목표 중심 실행과 검증

- 작업을 검증 가능한 완료 조건으로 바꾸고, 다단계 작업은 단계별 작업과 검증 방법을 간단히 제시한다.
- 버그 수정은 가능한 경우 재현 테스트를 먼저 추가하고, 리팩터링은 변경 전후에 적절한 회귀 검증을 수행한다. 자동화가 곤란하면 재현 절차와 수행한 수동 검증 또는 미수행 사유를 남긴다.
- 검증은 변경 위험에 비례해 수행하며, 실행하지 못한 테스트·기기·브라우저 검증은 통과한 것처럼 표현하지 않는다.
- 입력, 네트워크, 인증, 외부 API·네이티브 연동에서 현실적으로 발생 가능한 실패를 불가능한 경우로 간주해 오류 처리를 생략하지 않는다.

## 작은 변경의 예외 확인

AI가 작은 변경(파일 변경 5개 이하 또는 LOC 50 Line 이하)이라고 판단해도, 스스로 브랜치·PR 절차를 생략해서는 안 된다.
...
- 리뷰가 타당하든 타당하지 않든, 수정 필요 여부와 근거, 수정·재검증 계획을 사용자에게 먼저 정리해서 보고하고 승인을 기다린다.
...
- 사용자의 명시적인 PR 병합 요청이 있고 별도의 문서 갱신 요청이 없더라도, 병합 전에 프로젝트 내 문서의 정합성과 누락 여부를 확인한다.
- 구현·설정·API 계약·운영 절차 등과 문서 사이에 drift가 있거나 문서화가 누락된 내용이 발견되면 병합을 진행하지 않는다. 사용자에게 `문서 갱신이 필요하다`고 보고하고, `병합 전에 문서 갱신까지 작업한 뒤 병합하면 되는지` 확인받는다. 사용자가 승인하면 문서 갱신과 검증을 완료한 뒤 병합한다.
