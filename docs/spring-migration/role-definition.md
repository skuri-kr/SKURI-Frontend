...
- 도메인 상태 전이 규칙 관리
- 최초 프로필 완료 시 현재 이용약관 통합 동의를 `SIGNUP` 출처로 서버 DB에 자동 기록
...
예를 들어 클라이언트가 이용약관 체크 상태를 표시하더라도, 최초 프로필 완료 시 현재 약관 버전 동의 이력을 생성하고 광고 가능 여부를 응답하는 source of truth는 Spring 백엔드의 `member_terms_consents`다.
...
- Notice 권한 예시:
  ...
  - 공지 댓글 작성자 전용:
    ...
    - 앱 공지 댓글 위반 시 `403 NOT_APP_NOTICE_COMMENT_AUTHOR`
    - 학교 공지 댓글 위반 시 `403 NOT_NOTICE_COMMENT_AUTHOR`
  - 관리자 전용:
