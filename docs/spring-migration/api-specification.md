...
- 요청 수락으로 파티 인원이 정원(`maxMembers`)에 도달해도 파티 상태는 자동으로 `CLOSED` 전이되지 않습니다.
- 성공 시 파티 채팅방에 서버 생성 `SYSTEM` 메시지 `"{requesterName}님이 입장했어요."`가 추가됩니다. 닉네임이 비어 있거나 찾지 못하면 `"새 멤버가 입장했어요."`를 사용합니다.
- 정원 도달 시 남은 PENDING 동승 요청과 친구 초대만 `EXPIRED + CAPACITY_FULL`로 정리하며, 이미 열린 파티의 모집 상태는 유지됩니다.
- 모집 마감 `SYSTEM` 메시지와 상태 변경은 리더가 명시적으로 마감할 때만 생성·브로드캐스트됩니다.
...
- 동승 요청 수락으로 파티가 정원에 도달해도 `SYSTEM` 모집 마감 메시지와 파티 상태는 자동 변경되지 않습니다. 모집 마감·재개는 리더의 명시적 action에서만 처리됩니다.
