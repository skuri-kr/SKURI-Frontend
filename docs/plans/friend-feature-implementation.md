...
공지·이번 주 학식·커뮤니티 게시글 공유용 앱 링크 기반은 Frontend [#33](https://github.com/skuri-kr/SKURI-Frontend/pull/33)·[#34](https://github.com/skuri-kr/SKURI-Frontend/pull/34)에서 구현했다. 공개 공유 주소인 `link.skuri.kr`과 브라우저 fallback의 앱 실행 주소인 `open.skuri.kr` HTTPS 링크를 해석하고, cold·warm start 및 로그인·온보딩 대기 구간을 거쳐 대상 화면으로 이동한다. 후속 공유 링크 계약에서는 공지·게시물 원본 ID를 URL에 직접 넣지 않고 Backend Share API가 발급한 8자리 Base58 코드를 사용하며, 메인 진입 후 보호된 resolve API로 원본 ID를 복원한다. 기존 긴 링크와 deferred deep link는 지원하지 않는다. iOS Universal Links와 Android App Links 검증 설정은 두 도메인에 적용하며, URL 딥링크를 이용한 친구 추가는 여전히 이번 V1 제외 범위다.
